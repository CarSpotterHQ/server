...
(
    ...
    title       VARCHAR(255), -- 초대장 제목
    description TEXT,                  -- 초대장 설명
    start_time  TIMESTAMP    NOT NULL, -- 초대장 시작 시간
    created_at  TIMESTAMP    NOT NULL, -- 초대장 생성 시간
    updated_at  TIMESTAMP    NOT NULL, -- 초대장 수정 시간
    end_time    TIMESTAMP    NOT NULL, -- 초대장 종료 시간
    uuid        VARCHAR(256) NOT NULL  -- 초대장 UUID
);

CREATE TABLE tp_task
(
    id                BIGSERIAL PRIMARY KEY,
    name              VARCHAR(255) , -- 할일의 이름
    nickname          VARCHAR(255) , -- 사용자 이름
